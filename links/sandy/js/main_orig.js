...
function initialize(){
 ...
 //grab the results of a query for every tweet in database 
	...
	// window.setTimeout("setMap()", 2000); //getAllTweets needs time to finish before setMap begins. 
 }
...
function bindData (searchWord){
	...
	d3.json("./test.php?x="+searchWord, function(phpData) {
		
		var queryTweets = phpData; //why does 'evacuate return empty, but display colors on map???? 
...
function colorScale(frequenciesArray){ 
	...
	var color = d3.scale.quantile() //designate quantile scale 
		.range ([
			"#302C29", //grey-ish
			"#45455C",
...
function highlight (data, d){
	...
	if (d.properties.fillcolor){ //if the hexagon has a value, allow highlight
		...
		d3.selectAll(".boroughs")
			.moveToFront(); //move the boroughs to front so they aren't blocked 
		
		//label content: expressed variable, value, id of hexagon
		var labelAttribute = 	
		"<b>Frequency:</b><br/><h2>"+Math.round(freq*10000)/100+"%</h2>";
...
/*------------- hide or display About -------------*/
/*
aboutButton.onclick = function () {
	
	if (displayAbout == false){
		document.getElementById("right-triangle").style.display = "none";
		document.getElementById("left-triangle").style.display = "inline-block";
		document.getElementById("aboutContainer").style.display = "block";
		displayAbout = true;
	} else {
		document.getElementById("aboutContainer").style.display = "none";
		document.getElementById("right-triangle").style.display = "inline-block";
		document.getElementById("left-triangle").style.display = "none";
		displayAbout = false;
	};
}; 
*/
/*------------- end hide or display About -------------*/
...
controlsButton.onclick = function () {
	...
	if (displayControls == true){
		...
		document.getElementById("legend").style.right = "320px";
...
function cloudMake(){ 		//called at end of setMap !
	 var wordArray = [
		"power", "water", "train", "Obama", "Romney", "Christie", "work", "Sandy", "hurricane", "storm", "mayor", "cold", "NY", "subway", "crazy", "vote"/*, "evacuate", "rain", "crane", "lights", "dark", "Frankenstorm", "battery park", "climate", "batteries", "store", "supplies", "danger", "food", "subway", "MTA", "park",  "tree", "ConEd"*/];
