...
//                            0          1            2         3
$mailLabelFieldsString = "OwnerName,Adddress";
...
$sql = "SELECT distinct $mailLabelFieldsString FROM LGIM.PARCELS where ParcelNo in $str";
...
$results = array();
$ob_file = fopen('ownerMails.txt','w');

while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_NUMERIC) ) {

    if($row[1] == $row[3]){
		fwrite($ob_file,$row[0].",".$row[1].",".$row[2]);   
		fwrite($ob_file,"\r\n");
	}
	else{
