...
function drawGraphic(drawn) {
    ...
    symbols.polygon = new es.SimpleFillSymbol("solid", new es.SimpleLineSymbol("solid", new dc([206, 206, 206]), 2), new dc([255, 102, 0, 0.25]));
    ...
    if (layerToBuffer == "Drawing") {
        ...
        queryTask.execute(query, function (selectionGeometries) {
            ...
            if (currentSelection.length == 0 && layerToBuffer != parcelLayerID) {
                ...
                case "point":
                    var symbolSelect = new esri.symbol.SimpleMarkerSymbol(esri.symbol.SimpleMarkerSymbol.STYLE_CIRCLE, 10, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([0, 255, 255]), 1), new dojo.Color([0, 0, 255, 1]));
                    break;
                case "polyline":
                    var symbolSelect = new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([255, 165, 0]), 4);
                    break;
                case "polygon":
                    var symbolSelect = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([206, 206, 206]), 2), new dojo.Color([255, 102, 0, 0.05]));
                    break;
            }
...
function doBuffer() {
    ...
    params.unit = eval("esri.tasks.GeometryService." + dojo.byId("unitBuff").value);
...
function showBuffer(features) {
    $('.results.multipleBuffer').hide();
    var bufferSymbol = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([206, 206, 206, 0.65]), 2), new dojo.Color([0, 0, 255, 0.35]));
    map.graphics.clear();
...
function createTable(queryFeatures) {
    ...
    var querySymbol = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([206, 206, 206, 0.65]), 2), new dojo.Color([255, 102, 0, 0.35]));
